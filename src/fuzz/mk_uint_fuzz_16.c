...
static mk_inline void mk_uint_fuzz_16_from_long(unsigned char const* data)
{
	unsigned long n;
	uint16_t br;
	struct mk_uint16_s mr;
	...
	br = (uint16_t)n;
	mk_uint16_from_long(&mr, n);

	test(memcmp(&br, &mr, 16 / CHAR_BIT) == 0);
}

static mk_inline void mk_uint_fuzz_16_to_long(unsigned char const* data)
{
	uint16_t bx;
	unsigned long br;
	struct mk_uint16_s mx;
...
static mk_inline void mk_uint_fuzz_16_gt(unsigned char const* data)
{
	uint16_t ba;
	uint16_t bb;
	int br;
	...
	memcpy(&ma, data + 0 / CHAR_BIT, 16 / CHAR_BIT);
	memcpy(&mb, data + 16 / CHAR_BIT, 16 / CHAR_BIT);
	mr = mk_uint16_gt(&ma, &mb);

	test(memcmp(&br, &mr, sizeof(br)) == 0);
}

static mk_inline void mk_uint_fuzz_16_ge(unsigned char const* data)
{
	uint16_t ba;
	uint16_t bb;
	int br;
...
static mk_inline void mk_uint_fuzz_16_div(unsigned char const* data)
{
	uint16_t ba;
	uint16_t bb;
	uint16_t br;
	...
	memcpy(&ma, data + 0 / CHAR_BIT, 16 / CHAR_BIT);
	memcpy(&mb, data + 16 / CHAR_BIT, 16 / CHAR_BIT);
	mk_uint16_div(&mr, &ma, &mb);

	test(memcmp(&br, &mr, 16 / CHAR_BIT) == 0);
}

static mk_inline void mk_uint_fuzz_16_mod(unsigned char const* data)
{
	uint16_t ba;
	uint16_t bb;
	uint16_t br;
...
void mk_uint_fuzz_16(unsigned char const* data)
{
	...
	mk_uint_fuzz_16_from_long(data);
	mk_uint_fuzz_16_to_long(data);
	...
	mk_uint_fuzz_16_gt(data);
	mk_uint_fuzz_16_ge(data);
	...
	mk_uint_fuzz_16_div(data);
	mk_uint_fuzz_16_mod(data);
}
