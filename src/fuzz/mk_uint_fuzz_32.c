...
static mk_inline void mk_uint_fuzz_32_from_long(unsigned char const* data)
{
	unsigned long n;
	uint32_t br;
	struct mk_uint32_s mr;
	...
	br = (uint32_t)n;
	mk_uint32_from_long(&mr, n);

	test(memcmp(&br, &mr, 32 / CHAR_BIT) == 0);
}

static mk_inline void mk_uint_fuzz_32_to_long(unsigned char const* data)
{
	uint32_t bx;
	unsigned long br;
	struct mk_uint32_s mx;
...
static mk_inline void mk_uint_fuzz_32_mul(unsigned char const* data)
{
	uint32_t ba;
	uint32_t bb;
	uint32_t br;
	...
	memcpy(&ma, data + 0 / CHAR_BIT, 32 / CHAR_BIT);
	memcpy(&mb, data + 32 / CHAR_BIT, 32 / CHAR_BIT);
	mk_uint32_mul(&mr, &ma, &mb);

	test(memcmp(&br, &mr, 32 / CHAR_BIT) == 0);
}

static mk_inline void mk_uint_fuzz_32_div(unsigned char const* data)
{
	uint32_t ba;
	uint32_t bb;
	uint32_t br;
	...
	memcpy(&ma, data + 0 / CHAR_BIT, 32 / CHAR_BIT);
	memcpy(&mb, data + 32 / CHAR_BIT, 32 / CHAR_BIT);
	mk_uint32_div(&mr, &ma, &mb);

	test(memcmp(&br, &mr, 32 / CHAR_BIT) == 0);
}

static mk_inline void mk_uint_fuzz_32_mod(unsigned char const* data)
{
	uint32_t ba;
	uint32_t bb;
	uint32_t br;
...
void mk_uint_fuzz_32(unsigned char const* data)
{
	...
	mk_uint_fuzz_32_from_long(data);
	mk_uint_fuzz_32_to_long(data);
	...
	mk_uint_fuzz_32_mul(data);
	mk_uint_fuzz_32_div(data);
	mk_uint_fuzz_32_mod(data);
}
