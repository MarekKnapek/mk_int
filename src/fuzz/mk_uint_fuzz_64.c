...
static mk_inline void mk_uint_fuzz_64_from_long(unsigned char const* data)
{
	unsigned long n;
	uint64_t br;
	struct mk_uint64_s mr;
	...
	br = (uint64_t)n;
	mk_uint64_from_long(&mr, n);

	test(memcmp(&br, &mr, 64 / CHAR_BIT) == 0);
}

static mk_inline void mk_uint_fuzz_64_to_long(unsigned char const* data)
{
	uint64_t bx;
	unsigned long br;
	struct mk_uint64_s mx;
...
static mk_inline void mk_uint_fuzz_64_gt(unsigned char const* data)
{
	uint64_t ba;
	uint64_t bb;
	int br;
	...
	memcpy(&ma, data + 0 / CHAR_BIT, 64 / CHAR_BIT);
	memcpy(&mb, data + 64 / CHAR_BIT, 64 / CHAR_BIT);
	mr = mk_uint64_gt(&ma, &mb);

	test(memcmp(&br, &mr, sizeof(br)) == 0);
}

static mk_inline void mk_uint_fuzz_64_ge(unsigned char const* data)
{
	uint64_t ba;
	uint64_t bb;
	int br;
...
static mk_inline void mk_uint_fuzz_64_div(unsigned char const* data)
{
	uint64_t ba;
	uint64_t bb;
	uint64_t br;
	...
	memcpy(&ma, data + 0 / CHAR_BIT, 64 / CHAR_BIT);
	memcpy(&mb, data + 64 / CHAR_BIT, 64 / CHAR_BIT);
	mk_uint64_div(&mr, &ma, &mb);

	test(memcmp(&br, &mr, 64 / CHAR_BIT) == 0);
}

static mk_inline void mk_uint_fuzz_64_mod(unsigned char const* data)
{
	uint64_t ba;
	uint64_t bb;
	uint64_t br;
...
void mk_uint_fuzz_64(unsigned char const* data)
{
	...
	mk_uint_fuzz_64_from_long(data);
	mk_uint_fuzz_64_to_long(data);
	...
	mk_uint_fuzz_64_gt(data);
	mk_uint_fuzz_64_ge(data);
	...
	mk_uint_fuzz_64_div(data);
	mk_uint_fuzz_64_mod(data);
}
